...
int removePcc(){
  ...
  if(_laby[(_perso.y + 1) * W + _perso.x] == RGB (0,255,0)){
    _laby[(_perso.y + 1) * W + _perso.x] = 0;
    ++_perso.y;
    supp=1;
  }

  else if(_laby[(_perso.y - 1)* W + _perso.x] == RGB (0,255,0)){
    _laby[(_perso.y - 1)* W + _perso.x] = 0;
    --_perso.y;
    supp=1;
  }

  else if(_laby[_perso.y * W + _perso.x + 1] == RGB (0,255,0)){
    _laby[_perso.y * W + _perso.x + 1] = 0;
    ++_perso.x;
    supp=1;
  }
