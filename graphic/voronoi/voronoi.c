...
static void updateColor(int voronoiIndex);
static int drawCircleNoM(int x0, int y0, int r);
static int vdGrow(void);
 ...
 */
static inline int testAndPlot(int x, int y,int* ret) {
...
static void updateColor(int voronoiIndex)
{
    ...
    int x = ((int)_sites[voronoiIndex].x);
    int y = _background->h - ((int)_sites[voronoiIndex].y + 1); // + 1 évite une erreur de segmentation
    SDL_GetRGB(p[y * _background->w + x], _background->format, &r, &g, &b);
    _sites[voronoiIndex].color = RGB(r, g, b);
}
