 ...
 */
static void initForest(double density, GLfloat* lum) {
  GLuint i, wh = gl4dpGetWidth() * gl4dpGetHeight(), *map = gl4dpGetPixels();
  for (i = 0; i < wh; ++i)
    map[i] = (gl4dmURand() < density - (lum[i] * lum[i])) ? TREE : EMPTY;
}

/*!\brief allume le feu à tous les arbres du bord gauche. */
static void lightTheFire(void) {
  GLuint i, w = gl4dpGetWidth(), wh = w * gl4dpGetHeight(),
            *map = gl4dpGetPixels();
  for (i = 0; i < wh; i += w)
    map[i] = (map[i] == TREE) ? FIRE : map[i];
}

static void lightTheFire2(void) {
  GLuint i, j, w = gl4dpGetWidth(), wh = w * gl4dpGetHeight(),
               *map = gl4dpGetPixels();
  for (i = 0; i < w; i += 70) {
    j = rand() % wh;
    if (map[j] == TREE) {
...
static void cellularAutomatonStep(void) {
  GLuint i, w = gl4dpGetWidth(), h = gl4dpGetHeight(), wh = w * h,
            *map = gl4dpGetPixels();
    ...
    switch (map[i]) {
      ...
      case TREE: /* regardons les voisins */
      {
        ...
        for (j = 0; j < 8; ++j) {
          ...
          ny = y + d[j][1];
          if ((nx >= 0 && ny >= 0 && nx < w &&
               ny < h) /* pour ne pas sortir de la map */
              &&       /* ET ... */
              map_copy[ny * w + nx] ==
                  FIRE /* ... ce voisin <était> en feu */) {
...
int main(int argc, char** argv) {
  ...
  SDL_Surface* s = SDL_LoadBMP("forest.bmp");
  assert(s);
  ...
  if (!gl4duwCreateWindow(argc, argv,                   /* args du programme */
                          ...
                          10, 10, _background->w,
                          _background->h, /* x,y, largeur, heuteur */
  ...
  GLfloat* lum = gl4dpSDLSurfaceToLuminanceMap(_background);
